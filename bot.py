...
# === 1) Файл для хранения открытых сделок ===
...
# === 3) Инициализация Binance Futures через CCXT ===
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    logger.info(f"✳️  /start received from chat_id={update.effective_chat.id}")
...
# === 7) Стратегии (возвращают dict или None) ===
...
async def check_for_signals(ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for s in symbols:
        ...
        for strat in STRATEGIES:
            ...
            if not res:
                continue
            # отправляем сигнал на вход
            await ctx.bot.send_message(int(CHAT_ID), res["msg"])
            # сохраняем открытый трейд
            open_trades.append({**res, "opened_at": datetime.utcnow().isoformat()})
            daily_stats["total"] += 1
            save_trades(open_trades)
    # проверка на SL/TP по текущей цене
    still = []
...
def main() -> None:
    ...
    # ежедневная статистика в 09:00 Europe/Berlin
